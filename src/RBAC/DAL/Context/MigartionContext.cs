...
namespace RBAC.RBAC.DAL.Context;

/// <summary>
/// use 'dotnet ef migrations add -c SqlServerDbContext SqlInitialCreate -p../RBAC.DAL -o Migrations/SqlMigrations -s.' this commend to create migration script for SQL
/// </summary>
...
public class SqlServerDbContext(DbContextOptions<RbacSqlDbContext> options) : RbacSqlDbContext(options)
{
    protected override void OnConfiguring(DbContextOptionsBuilder options)
        => options.UseSqlServer(builder => builder.MigrationsAssembly("RBAC"));
}


/// <summary>
/// use 'dotnet ef migrations add -c PgSqlServerDbContext PostgreSqlInitialCreate -p ../RBAC.DAL -o Migrations/PostgreSqlMigrations -s .' this commend to create migration script for PgSql
/// </summary>
/// <param name="options"></param>
[ExcludeFromCodeCoverage(Justification = "Memory DB migration cannot be tested due to limitations.")]
...
{
    protected override void OnConfiguring(DbContextOptionsBuilder options)
        => options.UseNpgsql(builder => builder.MigrationsAssembly("RBAC"));
