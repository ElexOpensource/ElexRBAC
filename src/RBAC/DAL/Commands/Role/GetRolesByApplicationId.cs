...
public class GetRolesByApplicationId(Guid applicationId, bool isActive = true, bool includeTypeMaster = false) : IRequest<List<RBAC.DAL.Models.Role>>
{
...
public class GetRolesByApplicationIdSqlHandler(RbacSqlDbContext dbContext) : SqlRequestHandler<GetRolesByApplicationId, List<RBAC.DAL.Models.Role>>(dbContext)
{
    public override async Task<List<RBAC.DAL.Models.Role>> Handle(GetRolesByApplicationId request, CancellationToken cancellationToken)
    {
